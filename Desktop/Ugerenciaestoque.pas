...
begin
  ...
  dm.consultaproduto.SQL.Text :=
    'select * from produtos where prodquantidade < 5 and prodnome like ' +
    QuotedStr(edtbuscar.Text + '%');
  dm.consultaproduto.Open();
end;

procedure Tfrmgerenciaestoque.FormActivate(Sender: TObject);
begin
  dm.consultaproduto.Close;
  dm.consultaproduto.SQL.Text :=
    'select * from produtos where prodquantidade < 5 and prodnome like ' +
...
begin
  ...
  // Consulta o nome da subcategoria para enviar para a alteração
  dm.consultasubcategoria.Close;
  dm.consultasubcategoria.SQL.Text :=
    'select * from subcategoria where subcatcod = ' +
    QuotedStr(dm.consultaprodutoprodsubcategoria.AsString);
  dm.consultasubcategoria.Open;
  frmcadprodutos.edtsubcategoria.Text :=
    dm.consultasubcategoriasubcatnome.AsString;
  ...
  frmcadprodutos.iddoproduto_para_alterar :=
    dm.consultaprodutoprodcod.AsInteger;
  frmcadprodutos.iddasubcate := dm.consultaprodutoprodsubcategoria.AsInteger;
  // para não bugar markup não ficar sem valor
