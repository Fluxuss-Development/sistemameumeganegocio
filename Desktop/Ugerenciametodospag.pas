...
begin
  //ShowMessage('Você esqueceu um campo vazio !');
  mostraMensagem('Você esqueceu um campo vazio !', 'Você esqueceu um campo vazio !', 'OK');
  abort;
end;
...
begin
//retorna o select ao normal após identificar se existe um registro igual !
  dm.consultametodopag.Close;
  dm.consultametodopag.SQL.Text := 'select * from tiposdepagamentos';
  dm.consultametodopag.Open;
 // ShowMessage('Método de pagamento já cadastrado !!!');
  mostraMensagem('Método de pagamento já cadastrado !!!', 'Método de pagamento já cadastrado !!!', 'OK');
  abort;
end;
 ...
 begin
  // ShowMessage('Novo método de pagamento cadastrado com sucesso !');
   mostraMensagem('Novo método de pagamento cadastrado com sucesso !', 'Novo método de pagamento cadastrado com sucesso !', 'OK');
     // notificação do windows 10
  notificacao := centraldenotifialteracao.CreateNotification;
  try
    notificacao.Name := 'Meu mega Negócio';
    notificacao.Title := 'Cadastro de método de pagamento !';
  ...
  //retorna o select ao normal após cadastrar para evitar bugs demasiados !
  dm.consultametodopag.Close;
  dm.consultametodopag.SQL.Text := 'select * from tiposdepagamentos';
  dm.consultametodopag.Open;
...
begin
 ...
 // ShowMessage('Alterado com sucesso, consulte novamente para ver as mudanças!');
  mostraMensagem('Alterado com sucesso, consulte novamente para ver as mudanças!', 'Alterado com sucesso, consulte novamente para ver as mudanças!', 'OK');
  dm.consultametodopag.Close;
  edtnomemetodopag.Text := '';
  ...
  try
    notificacao.Name := 'Meu mega Negócio';
    notificacao.Title := 'Alteração de método de pagamento !';
    notificacao.AlertBody :=
      'O Método de pagamento foi alterado com sucesso !';
    centraldenotifialteracao.ScheduleNotification(notificacao);
    centraldenotifialteracao.PresentNotification(notificacao);
  finally
    notificacao.Free;
